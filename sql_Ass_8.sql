drop database if exists sql_ass_8;
create database if not exists sql_ass_8;
use sql_ass_8;
...
select Name,EmployeeID from physician left join trained_in on physician.EmployeeID=trained_in.Physician where Physician is null ;
...
select physician.Name as physician_name,procedures.Name procedures_name ,undergoes.DateUndergoes as date_procedure ,patient.Name as patients 
from physician right join undergoes on physician.EmployeeID=undergoes.Physician
...
select physician.Name as physician_name,EmployeeID,trained_in.CertificationExpires,DateUndergoes from physician 
...
where trained_in.Physician is not null group by physician_name;
...
select procedures.Code, physician.Name as physician_name,procedures.Name as procedure_name,patient.Name as patient_name,trained_in.CertificationExpires,undergoes.DateUndergoes from physician 
...
join procedures on procedures.Code=undergoes.Physician
join patient on patient.SSN=undergoes.Patient
where trained_in.Physician is not null and certificationexpires < dateundergoes group by physician_name;

select p.name,certificationexpires,dateundergoes
from undergoes as u
left join physician as p on (u.Physician = p.EmployeeID)
left join trained_in as t on (u.Procedures = t.Treatment and u.Physician = t.Physician)
...
select patient.Name ,appointment.physician,physician.Name as physician_name,nurse.Name as nurse_name,appointment.Start,appointment.End,appointment.ExaminationRoom
 from  patient left join appointment on appointment.Patient=patient.SSN 
 ...
 where appointment.physician not in (select EmployeeID from pcp_t);

select * from nurse,appointment;
-- create table pcp_t as select * from physician where EmployeeID in (select pcp from patient);
-- drop table pcp_t;
-- select * from pcp_t;
use sql_ass_8;
...
select * from stay as s, undergoes as u where s.StayID = u.Stay and s.Patient <> u.Patient;
select * from stay;

-- 8.7 Obtain the names of all the nurses who have ever been on call for room 123.
select nurse.Name,nurse.EmployeeID,roomnumber from room left join on_call on room.BlockCode=on_call.BlockCode
left join nurse on nurse.EmployeeID=on_call.Nurse where RoomNumber=123;
select *  from on_call;
...
select count(AppointmentID) as  number_of_appointment,examinationroom from appointment group by examinationroom;
...
select patient.pcp as pcp_id, patient.Name as patient_name,pcp_t.Name as pcp_name from patient join pcp_t on patient.pcp=pcp_t.EmployeeID
where patient.pcp in (select physician from prescribes); 

    -- The patient has undergone a procedure with a cost larger that $5,000
select procedures.code, patient.Name as patient_name,pcp_t.Name as pcp_name from patient join pcp_t on patient.pcp=pcp_t.EmployeeID
 join undergoes on patient.pcp=undergoes.physician join procedures on procedures.Code=undergoes.procedures
where procedures.cost>5000;
select * from appointment,nurse;
 
 -- The patient has had at least two appointment where the nurse who prepped the appointment was a registered nurse.
select patient.Name as patient_name,count(AppointmentID) as t_app from patient join appointment on patient.SSN=appointment.patient
 join nurse on nurse.EmployeeID=appointment.PrepNurse
  where nurse.registered =1 group by patient_name having count(AppointmentID)>=2;
 
 -- The patient's primary care physician is not the head of any department.
select physician.Name from department left join pcp_t on department.Head=pcp_t.EmployeeID
left join physician on physician.EmployeeID=pcp_t.EmployeeID;
