drop database if exists sql_ass_7;
create database if not exists sql_ass_7;
use sql_ass_7; 
...
select client.name, package.weight from client join package on client.AccountNumber=package.recipient where weight=1.5;

-- 7.2 What is the total weight of all the packages that he sent?
select client.name, sum(package.weight) as total_weight from client join package on client.AccountNumber=package.recipient where client.name="Al Gore's Head" group by client.name;
